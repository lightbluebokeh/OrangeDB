...
namespace BytesIO {
    ...
    template<typename T>
    T read(bytes_t bytes) { return *(T*)bytes; }
    ...
    template<typename T>
    std::enable_if_t<is_byte_v<T>, size_t> read_bytes(bytes_t bytes, std::vector<T>& vec, size_t n) {
        vec = std::vector<T>(bytes, n);
        return n;
    }

    template<typename T>
    std::enable_if_t<is_byte_v<T>, size_t> read_bytes(bytes_t bytes, std::basic_string<T>& str, size_t n) {
