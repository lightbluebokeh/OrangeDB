...
namespace Orange::parser {
    ...
    inline std::istream& operator>>(std::istream& is, data_value& value) {
        ...
        switch (data_value_kind(which)) {
            case data_value_kind::Null:  // do nothing
                break;
            case data_value_kind::Int: is >> value.to_int(); break;
            case data_value_kind::String: is >> value.to_string(); break;
            case data_value_kind::Float: is >> value.to_float(); break;
        }
        return is;
    }
    inline std::ostream& operator<<(std::ostream& os, const data_value& value) {
        ...
        switch (data_value_kind(value.value.which())) {
            case data_value_kind::Null:  // do  nothing
                break;
            case data_value_kind::Int: os << ' ' << value.to_int(); break;
            case data_value_kind::String: os << ' ' << value.to_string(); break;
            case data_value_kind::Float: os << ' ' << value.to_float(); break;
        }
        return os;
    }
    ...
    struct idx_stmt {
        ...
        const alter_add_idx_stmt& alter_add() const { return boost::get<alter_add_idx_stmt>(stmt); }
    ...
    struct alter_stmt {
        ...
        add_primary_key_stmt& add_primary_key() { return boost::get<add_primary_key_stmt>(stmt); }
            ...
            case data_value_kind::String:
                switch (type.kind) {
                    ...
                    case orange_t::Char: {
                        ...
                        orange_assert(int(str.length()) <= type.int_value(),
                    ...
                    case orange_t::Date: {
                        ...
                        std::tm check = t;
                        orange_assert(!ss.fail() && std::mktime(&check) > 0, "invalid date");
